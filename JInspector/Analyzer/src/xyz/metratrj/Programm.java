    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            for (ClassFile classFile : arr) {
                ...
                ArrayList<AccessFlags> classAccessFlags = new ArrayList<>();
                ...
                for (field_info fieldInfo : classFile.getFields()) {
                    ...
                    if ((accessFlags & ClassFile.ACC_PUBLIC) != 0) { fieldAccessFlags.add(AccessFlags.PUBLIC); }
                    if ((accessFlags & ClassFile.ACC_PRIVATE) != 0) { fieldAccessFlags.add(AccessFlags.PRIVATE); }
                    if ((accessFlags & ClassFile.ACC_PROTECTED) != 0) { fieldAccessFlags.add(AccessFlags.PROTECTED); }
                    if ((accessFlags & ClassFile.ACC_STATIC) != 0) { fieldAccessFlags.add(AccessFlags.STATIC); }
                    if ((accessFlags & ClassFile.ACC_FINAL) != 0) { fieldAccessFlags.add(AccessFlags.FINAL); }
                    if ((accessFlags & ClassFile.ACC_VOLATILE) != 0) { fieldAccessFlags.add(AccessFlags.VOLATILE); }
                    if ((accessFlags & ClassFile.ACC_TRANSIENT) != 0) { fieldAccessFlags.add(AccessFlags.TRANSIENT); }
                    if ((accessFlags & ClassFile.ACC_SYNTHETIC) != 0) { fieldAccessFlags.add(AccessFlags.SYNTHETIC); }
                    if ((accessFlags & ClassFile.ACC_ENUM) != 0) { fieldAccessFlags.add(AccessFlags.ENUM); }

                    String fieldName = classFile.getConstantPoolItem(fieldInfo.getName_index(), CONSTANT_Utf8_info.class).getValue();

                    System.out.printf("\nFieldName: %s\nFlags: %s\n\n", fieldName, fieldAccessFlags.toString());
